
<!-- MODAL FORMULARIO REGISTRAR-->
<div class="modal fade" id="registrarModal" tabindex="-1" aria-labelledby="registrarModalLabel" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
    <div class="modal-header">
        <h2 class="modal-title text-center" id="registrarModalLabel">Formulario</h2>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
        </button>
    </div>
    <div class="modal-body">
        <div class="container">
            <!-- FORMULARIO -->
            <form action="">
            <div class="row  mt-2">
                <div class="col-md-4">
                    <input type="text" name="" class="form-control" placeholder="Nombre">
                </div>
                <div class="col-md-4">
                    <input type="text" name="" class="form-control" placeholder="Apellido">
                </div>
                <div class="col-md-4">
                    <input type="text" name="" class="form-control" placeholder="CI">
                </div>
            </div>

            <div class="row mt-4">
                <div class="col-md-4">
                    <input type="email" name="" class="form-control" placeholder="Correo">
                </div>
                <div class="col-md-4">
                    <input type="number" name="" class="form-control" placeholder="Teléfono">
                </div>
                <div class="col-md-4">
                    <select name="" id="" class="custom-select">
                    <option value="">Número de apartamento</option>
                    <option value="">104</option>
                    <option value="">105</option>
                    <option value="">106</option>
                    </select>
                </div>
            </div>

            <div class="row mt-4">
                <div class="col-md-4">
                    <input type="number" name="" class="form-control" placeholder="Residentes">
                </div>
                <div class="col-md-4">
                    <input type="number" name="" class="form-control" placeholder="Mascotas">
                </div>
                <div class="col-md-4"></div>
            </div>


            <div class="row">
                <div class="col-md-4"></div>
                <div class="col-md-5"></div>
                <div class="col-md-3">
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary">Registrar</button>
                </div>
                </div>
            </div>
        </form>
        </div>
    <!-- FIN CONTENIDO DEL MODAL -->
    </div>
    </div>
</div>
</div>


<!-- MODAL FORMULARIO EDITAR-->
<div class="modal fade" id="editarModal" tabindex="-1" aria-labelledby="modalEditarLabel" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
    <div class="modal-header">
        <h2 class="modal-title text-center" id="modalEditarLabel">Formulario Editar</h2>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
        <span aria-hidden="true">&times;</span>
        </button>
    </div>
    <div class="modal-body">
        <div class="container">
            <!-- FORMULARIO -->
            <form action="">
            <div class="row  mt-2">
                <div class="col-md-4">
                    <input type="text" name="" class="form-control" placeholder="Nombre">
                </div>
                <div class="col-md-4">
                    <input type="text" name="" class="form-control" placeholder="Apellido">
                </div>
                <div class="col-md-4">
                    <input type="text" name="" class="form-control" placeholder="CI">
                </div>
            </div>

            <div class="row mt-4">
                <div class="col-md-4">
                    <input type="email" name="" class="form-control" placeholder="Correo">
                </div>
                <div class="col-md-4">
                    <input type="number" name="" class="form-control" placeholder="Teléfono">
                </div>
                <div class="col-md-4">
                    <select name="" id="" class="custom-select">
                    <option value="">Número de apartamento</option>
                    <option value="">104</option>
                    <option value="">105</option>
                    <option value="">106</option>
                    </select>
                </div>
            </div>

            <div class="row mt-4">
                <div class="col-md-4">
                    <input type="number" name="" class="form-control" placeholder="Residentes">
                </div>
                <div class="col-md-4">
                    <input type="number" name="" class="form-control" placeholder="Mascotas">
                </div>
                <div class="col-md-4"></div>
            </div>


            <div class="row">
                <div class="col-md-4"></div>
                <div class="col-md-5"></div>
                <div class="col-md-3">
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary">Guardar</button>
                </div>
                </div>
            </div>
        </form>
        </div>
    <!-- FIN CONTENIDO DEL MODAL -->
    </div>
    </div>
</div>
</div>
